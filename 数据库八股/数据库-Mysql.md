...
- 对于读未提交，可以读到未提交事物修改的数据，所以直接读取最新数据就可以
- 对于串行化：加读写锁即可
- 对于读提交和可重复读通过read view来实现
  - 读提交隔离级别是在每个select都会生成一个新的read view，也意味着事物期间的多次读取同一条数据，前后两次读到的数据kennel会出现不一致，因为可能期间另一个事物修改了该记录，并提交了事物
  - 可重复读隔离级别是启动事物是生成一个read view，然后整个事物期间都在用这个read view 这样就可以保证了在事物期间读到的数据都是在事物启动前的记录

#### 7 Read view 在MVCC中是如何工作的

1:事物启动：当一个事物启动时，系统会为该事物创建一个read view

2:read view 包含的信息

![135](MySql_Picture/135.jpeg)

对于聚簇索引有以下两个隐藏列：

- trx-id，当一个事物对某条聚簇索引记录进行改动时，就会把该条事物的id记录在trx-id隐藏列里
- roll- pointer，每次对聚簇索引记录进行改动，都会把旧版本的记录写入undo日志中，然后这个隐藏列是个指针，指向每一条旧版本的记录，通过这个可以查看旧版本
